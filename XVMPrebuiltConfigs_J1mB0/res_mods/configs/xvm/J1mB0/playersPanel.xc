 ...
 */
{
  ...
  "def": {
    ...
    "clanIcon": {
      "enabled": false,
      "x": 100, "y": 0, "width": 24, "height": 24, "align": "center", "alpha": 75, "bindToIcon": true,
      "src": "{{clanicon}}"
    ...
    "hpBarBg": { "hotKeyCode": 56, "onHold": "true", "visibleOnHotKey": true, "x": 96, "y": 6, "width": 82, "bindToIcon": true, "height": 14, "bgColor": "0x000000", "alpha": "{{alive?48|0}}" },
    // HP bar.
    // Индикатор HP.
    "hpBar": { "hotKeyCode": 56, "onHold": "true", "visibleOnHotKey": true, "x": 97, "y": 7, "bindToIcon": true, "width": "{{hp-ratio:80}}", "height": 12, "bgColor": "{{player?#FEFA1F|{{c:system}}}}", "alpha": "{{alive?32|0}}" },
    // Remaining HP.
    // Оставшиеся HP.
    "hp": {
      "hotKeyCode": 56, "onHold": "true", "visibleOnHotKey": true, "bindToIcon": true, "alpha": "{{alive?100|0}}",
      "x": 96, "width": 82, "y": 4,
      "textFormat": { "font": "$FieldFont", "size": 11, "color": "0xE6E6E6", "bold": "true", "align": "center" },
      "format": "<font alpha='{{alive?{{ready?#FF|#80}}|#80}}'>{{alive?{{hp|--}}|{{l10n:Destroyed}}}}</font>",
      "shadow": { "enabled": true, "color": "0x000000", "alpha": 100, "blur": 4, "strength": 1, "distance": 0, "angle": 45 }
    }
  },
  // Parameters of the Players Panels ("ears").
  // Параметры панелей игроков ("ушей").
  "playersPanel": {
    ...
    "large": {
      ...
      // Display format for player nickname (macros allowed, see macros.txt).
      // Формат отображения имени игрока (допускаются макроподстановки, см. macros_ru.txt).
      "nickFormatLeft": "<font color='{{c:winrate}}' alpha='{{alive?#FF|#80}}'>{{rating%2d~%}}</font> <font alpha='#A0'>{{clan}}</font> {{name}}",
      "nickFormatRight": "{{name}} <font alpha='#A0'>{{clan}}</font> <font color='{{c:winrate}}' alpha='{{alive?#FF|#80}}'>{{rating%2d~%}}</font>",
      // Shadow for player name field (default null = no shadow, as in vanillas client).
      // Тень для поля имени игрока (по умолчанию null = без тени, как в чистом клиенте).
      "nickShadowLeft": null,
      "nickShadowRight": null,
