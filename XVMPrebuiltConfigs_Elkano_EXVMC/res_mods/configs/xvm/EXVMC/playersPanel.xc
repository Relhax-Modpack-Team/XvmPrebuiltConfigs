 ...
 */
{
  ...
  "def": {
    ...
    "clanIcon": {
      "enabled": true,
      "x": 0, "y": 0, "width": 24, "height": 24, "align": "center", "alpha": 90, "bindToIcon": true,
      "src": "{{clanicon}}"
    ...
    "hpBar2Bg": {
      "x": 2, "y": 22, "valign": "center", "height":  2, "width": "{{hp:300}}",     "bgColor": "0x000000", "alpha": 40
	},
    "hpBar2": {
      "x": 2, "y": 22, "valign": "center", "height":  2, "width": "{{hp-max:300}}", "bgColor": "{{player?#FFDD33|{{c:system}}}}", "alpha": 50
	}
  },
  // Parameters of the Players Panels ("ears").
  // Параметры панелей игроков ("ушей").
  "playersPanel": {
    ...
    "medium2": {
      ...
      "extraFieldsLeft": [
        ...
        ${"def.xvmUserMarker"},
        ${"def.xmqpServiceMarker"}
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp-max:125}}", "bgColor": 0,          "alpha": 40 },
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp:125}}",     "bgColor": "0x13C313", "alpha": 50 }
//        { "x": 0, "y":  2, "valign": "center", "height": 21, "width": 3,                "bgColor": "0x13C313", "alpha": "{{alive?75|0}}" }
      ],
      // Set of formats for right panel (extended format supported, see above).
      // Набор форматов для правой панели (поддерживается расширенный формат, см. выше).
      "extraFieldsRight": [
        ...
        ${"def.enemySpottedMarker"}
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp-max:125}}", "bgColor": 0,          "alpha": 40 },
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp:125}}",     "bgColor": "0xFF0F0F", "alpha": 50 }
//        { "x": 0, "y":  2, "valign": "center", "height": 21, "width": 3,                "bgColor": "0xFF0F0F", "alpha": "{{alive?75|0}}" }
      ]
    },
    ...
    "large": {
      ...
      // Display format for player nickname (macros allowed, see macros.txt).
      // Формат отображения имени игрока (допускаются макроподстановки, см. macros.txt).
      "nickFormatLeft": "<font face='mono' size='13' color='{{c:winrate}}' alpha='{{alive?#FF|#80}}'>{{winrate%2d~%|--%}}</font> <img src='xvm://res/icons/xvm/xvm-user-{{xvm-user|none}}.png'> <font color='{{c:xr}}'>{{name%.20s~..}}</font> <font alpha='#A0' face='mono' size='10'>{{clan}}</font>",
      "nickFormatRight": "<font alpha='#A0' face='mono' size='10'>{{clan}}</font> <font color='{{c:xr}}'>{{name%.20s~..}}</font> <img src='xvm://res/icons/xvm/xvm-user-{{xvm-user|none}}.png'> <font face='mono' size='13' color='{{c:winrate}}' alpha='{{alive?#FF|#80}}'>{{winrate%2d~%|--%}}</font>",
      // Shadow for player name field (default null = no shadow, as in vanillas client).
      // Тень для поля имени игрока (по умолчанию null = без тени, как в чистом клиенте).
      "nickShadowLeft": null,
      "nickShadowRight": null,
      ...
      // Display format for vehicle name (macros allowed, see macros.txt).
      // Формат отображения названия танка (допускаются макроподстановки, см. macros.txt).
      "vehicleFormatLeft": "<font color='{{c:t-battles}}'>{{vehicle}}</font>",
      "vehicleFormatRight": "<font color='{{c:t-battles}}'>{{vehicle}}</font>",
      // Shadow for vehicle name field (default null = no shadow, as in vanillas client).
      // Тень для поля названия танка (по умолчанию null = без тени, как в чистом клиенте).
      "vehicleShadowLeft": null,
      "vehicleShadowRight": null,
      ...
      "extraFieldsLeft": [
        ...
        ${"def.xvmUserMarker"},
        ${"def.xmqpServiceMarker"}
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp-max:300}}", "bgColor": 0,          "alpha": 40 },
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp:300}}",     "bgColor": "0x13C313", "alpha": 50 }
//        { "x": 0, "y":  2, "valign": "center", "height": 21, "width": 3,                "bgColor": "0x13C313", "alpha": "{{alive?75|0}}" }
      ],
      // Set of formats for right panel (extended format supported, see above).
      // Набор форматов для правой панели (поддерживается расширенный формат, см. выше).
      "extraFieldsRight": [
        ...
        ${"def.enemySpottedMarker"}
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp-max:300}}", "bgColor": 0,          "alpha": 40 },
//        { "x": 2, "y": 21, "bindToIcon": true, "valign": "center", "height":  2, "width": "{{hp:300}}",     "bgColor": "0xFF0F0F", "alpha": 50 }
//        { "x": 0, "y":  2, "valign": "center", "height": 21, "width": 3,                "bgColor": "0xFF0F0F", "alpha": "{{alive?75|0}}" }
      ]
    }
  }
