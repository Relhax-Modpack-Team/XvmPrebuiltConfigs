 ...
 */
{
  ...
  "battleLoading": {
    ...
    "formatLeftNick": "<img src='xvm://res/icons/flags/{{flag|empty}}.png' width='16' height='13'><font alpha='{{alive?#FF|#80}}' size='13'>{{name%.16s~..}}<font color='#FFCC66'>{{clan}}</font></font>",
    // Display format for the right panel (macros allowed, see macros.txt).
    // Формат отображения для правой панели (допускаются макроподстановки, см. macros.txt).
    "formatRightNick": "<font alpha='{{alive?#FF|#80}}' size='13'>{{name%.16s~..}}<font color='#FFCC66'>{{clan}}</font></font><img src='xvm://res/icons/flags/{{flag|empty}}.png' width='16' height='13'>",
    // Display format for the left panel (macros allowed, see macros.txt).
    // Формат отображения для левой панели (допускаются макроподстановки, см. macros.txt).
    "formatLeftVehicle": "<font face='mono' size='12'><font color='{{c:kb}}'>{{kb%2d~k|--k}}</font> <font color='{{c:t-battles}}'>{{t-hb%2d~h|--h}} </font> <b><font color='{{c:wn8}}'>{{wn8%4d|----}}</font> </b> <font color='{{c:winrate}}'>{{winrate%2d~%|--%}}</font> <font color='{{c:t-winrate}}'>{{t-winrate%2d~%|--%}}</font></font>",
    // Display format for the right panel (macros allowed, see macros.txt).
    // Формат отображения для правой панели (допускаются макроподстановки, см. macros.txt).
    "formatRightVehicle": "<font face='mono' size='12'><font color='{{c:kb}}'>{{kb%2d~k|--k}}</font> <font color='{{c:t-battles}}'>{{t-hb%2d~h|--h}} </font> <b><font color='{{c:wn8}}'>{{wn8%4d|----}}</font> </b> <font color='{{c:winrate}}'>{{winrate%2d~%|--%}}</font> <font color='{{c:t-winrate}}'>{{t-winrate%2d~%|--%}}</font></font>",
